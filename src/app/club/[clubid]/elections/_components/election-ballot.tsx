    ...
    <div className="space-y-6">
      ...
      <div className="rounded-xl border border-blue-100 bg-blue-50 p-4 text-sm text-blue-800">
        <div className="flex flex-wrap gap-x-6 gap-y-3">
          {election.opens_at && <span className="inline-flex items-center gap-2"><Clock3 className="size-4" />Opened {new Date(election.opens_at).toLocaleString("en-US", { month: "long", day: "numeric", hour: "numeric", minute: "2-digit" })}</span>}
          <span className="inline-flex items-center gap-2"><Clock3 className="size-4" />Closes {new Date(election.closes_at).toLocaleString("en-US", { month: "long", day: "numeric", hour: "numeric", minute: "2-digit" })}</span>
          <span>{election.positions.length} position{election.positions.length === 1 ? "" : "s"}</span>
          <span>{election.ballots_cast} of {election.eligible_voters} members have voted</span>
          <span className="sm:ml-auto inline-flex items-center gap-2"><LockKeyhole className="size-4" />Live results are private</span>
        </div>
      </div>
      {election.positions.map((position, index) => (
        <section key={position.id} className="rounded-2xl border border-slate-200 bg-white p-6 sm:p-7">
          <p className="text-[11px] font-bold uppercase tracking-wider text-primary">Position {index + 1} of {election.positions.length}</p><h3 className="mt-1 text-xl font-bold text-slate-950">{position.title}</h3><p className="mt-2 max-w-3xl text-sm leading-relaxed text-slate-600">{position.description}</p><p className="mt-2 text-xs text-slate-400">Current holder: {position.current_holder_name}</p>
