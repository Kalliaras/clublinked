...
export default async function ProfilePage({ params }: { params: Promise<{ profileid: string }> }) {
  ...
  const { data: userInterests } = await supabase
    .from('user_interests').select('interest_id').eq('user_id', profileid);
  const interestIds = userInterests?.map(i => i.interest_id).filter(Boolean) || [];
  const { data: interestTags } = interestIds.length > 0
    ? await supabase.from('interest_tags').select('id, name').in('id', interestIds)
    : { data: [] };
  const interestsMap = new Map(interestTags?.map(t => [t.id, t.name]) || []);
  const interests: Interest[] = userInterests?.map(i => ({ name: interestsMap.get(i.interest_id) || 'Unknown Interest' })) || [];

  const { data: userSkills } = await supabase
    .from('user_skills').select('skill_id').eq('user_id', profileid);
  const skillIds = userSkills?.map(s => s.skill_id).filter(Boolean) || [];
  const { data: skillTags } = skillIds.length > 0
    ? await supabase.from('skill_tags').select('id, name').in('id', skillIds)
    : { data: [] };
